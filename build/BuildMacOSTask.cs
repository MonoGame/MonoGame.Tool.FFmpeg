...
public sealed class BuildMacOSTask : FrostingTask<BuildContext>
{
    ...
    private static void BuildArm64(BuildContext context)
    {
        ...
        {
            {"CFLAGS", $"-mmacosx-version-min=10.15 -w -arch arm64 -I{dependencyDir}/include"},
            {"CPPFLAGS", $"-mmacosx-version-min=10.15 -arch arm64 -I{dependencyDir}/include"},
            {"CXXFLAGS", "-mmacosx-version-min=10.15 -arch arm64"},
            {"LDFLAGS", $"-mmacosx-version-min=10.15 -arch arm64 -L{dependencyDir}/lib"},
    ...
    private static void BuildX64(BuildContext context)
    {
        ...
        var envVariables = new Dictionary<string, string>
        {
            {"CFLAGS", $"-mmacosx-version-min=10.15 -w -arch x86_64 -I{dependencyDir}/include"},
            {"CPPFLAGS", $"-mmacosx-version-min=10.15 -arch x86_64 -I{dependencyDir}/include"},
            {"CXXFLAGS", "-mmacosx-version-min=10.15 -arch x86_64"},
            {"LDFLAGS", $"-mmacosx-version-min=10.15 -arch x86_64 -L{dependencyDir}/lib"},
            {"PKG_CONFIG_PATH", $"{dependencyDir}/lib/pkgconfig"}
        };
